...
本仓库 fork 自上游，并在上游基础上追加了少量修复。这些修复的现象、根因、改动与实测数据，
以及从上游同步后如何确认补丁还在，统一记录在 **[PATCHES.md](./PATCHES.md)**。

> 补丁都改在 `src/` 源码里，只要源码没被上游覆盖，重新执行 `docker compose up -d --build` 依然带着修复。
